...
gui = ti.GUI("Taichi Elements", res=(res * 15, res * 6), background_color=0x112F41)
...
for i in range(3):
    ...
    mpm.add_ellipsoid(center=[0.275 + space * i, 0.4], radius=0.05, material=MPMSolver.material_elastic)
...
def vibrate(t: ti.f32, dt: ti.f32):
    for I in ti.grouped(mpm.grid_m):
        ...
        if p[1] > 0.8:
            ...
            mpm.grid_v[I] = ti.Vector([omega * 0.02 * ti.cos(t * omega), 0.0])
...
for frame in range(500):
    ...
    for i in range(3):
        gui.text(f'omega={(i + 1) * omega_step:.2f}/sec', (space / 2.5 * i + 0.05, 0.1))
